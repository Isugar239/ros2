...
class TurtleFollowerSpawner(Node):
    def __init__(self):
        ...
        # Целевая дистанция следования (метров) — нужна уже на этапе спавна
        self.follow_distance = 1.0

        # Подписчик на позу лидера (должен быть создан ДО спавна, чтобы знать где появиться)
        ...
        # Текущие позы и флаг получения позы лидера
        self.leader_pose = Pose()
        self.follower_pose = Pose()
        self._leader_pose_received = False
        ...
        # Дождаться первой позы лидера, чтобы заспавнить ведомую позади него
        wait_attempts = 0
        while not self._leader_pose_received and wait_attempts < 100:
            rclpy.spin_once(self, timeout_sec=0.1)
            wait_attempts += 1

        if not self._leader_pose_received:
            self.get_logger().warn('Leader pose not received in time; spawning follower at provided defaults')
            spawn_x = float(self.follower_x)
            spawn_y = float(self.follower_y)
            spawn_theta = float(self.follower_theta)
        else:
            # Позади лидера на follow_distance по его ориентации
            spawn_x = self.leader_pose.x - self.follow_distance * math.cos(self.leader_pose.theta)
            spawn_y = self.leader_pose.y - self.follow_distance * math.sin(self.leader_pose.theta)
            spawn_theta = float(self.leader_pose.theta)

        # Спауним ведомую черепаху
        req = Spawn.Request()
        req.x = float(spawn_x)
        req.y = float(spawn_y)
        req.theta = float(spawn_theta)
    ...
    def leader_pose_callback(self, msg: Pose):
        ...
        self._leader_pose_received = True
